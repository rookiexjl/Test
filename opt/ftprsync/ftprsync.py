...
class FTPSync(object):
	def __inint__(self, host, username, password, ftp_base_dir,
	...
	def get_dirs_files(self):
		...
		files = [f.split(None, 8)[-1] for f in dir_res if f.startwith('-')]
		dirs = [f.split(None, 8)[-1] for f in dir_res if f.startwith('d')]
		return (files, dirs)
...
if __name__ == '__,main__':
	...
	parser.add_option("-r", "--remote_dir", dest="remote-dir",
	...
	(options, args) = passer.parse_args()
	f = FTPSyn(option.host, option.username, option.password,
		option.remote_dir, option.local_dir, option.delete)
	f.run()
